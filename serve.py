#!/usr/bin/env python3
"""Запускалка diadoc-mcp-ru, поднимающая себе окружение сама.
...
текущему процессу и стартует сервер. Дальше запуск мгновенный.
    ...
    python3 serve.py diadoc               # запустить сервер
    python3 serve.py diadoc --selfcheck   # проверить установку и выйти

Нужна именно она, а не `uvx diadoc-mcp-ru`: бандл .mcpb для Claude Desktop
...
VENV = Path(os.environ.get("DIADOC_MCP_VENV", HERE / ".venv"))
DEPS = ["schema-mcp-core>=0.3.0,<1"]
SERVICES = {"diadoc": "diadoc_mcp.server"}


def _log(msg: str) -> None:
    print(f"[diadoc-mcp-ru] {msg}", file=sys.stderr, flush=True)
...
def main() -> None:
    ...
    sys.path.insert(0, str(HERE))  # чтобы импортировался diadoc_mcp/ из бандла
