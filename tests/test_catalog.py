...
CATALOG = ROOT / "diadoc_mcp" / "endpoints.yaml"
SVC = "diadoc"
...
def test_entities_file_ships_with_the_catalog():
    """Карта сущностей это половина поиска и весь инструмент diadoc_map. Пока
    файла не было, map молча отдавал пустоту во всех пяти серверах."""
    ents = yaml.safe_load((ROOT / "diadoc_mcp" / "entities.yaml").read_text(encoding="utf-8"))
...
def test_live_questions_hit_the_right_method():
    ...
    idx = EntityIndex.load(ROOT / "diadoc_mcp" / "entities.yaml")
    catalog = Catalog.from_yaml(CATALOG, entities=idx)
    for question, expected in [('входящие документы', 'diadoc_get_document'), ('список контрагентов', 'diadoc_get_counteragent_groups'), ('машиночитаемая доверенность', 'diadoc_register_power_of_attorney_result')]:
